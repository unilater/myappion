...
export class DataService {
  ...
  // =======================
  // UPLOAD PREMIUM (user-centric, retro-compatibile)
  // =======================
  uploadFilePremium(
    userId: number,
    ...
    // campi legacy (restano supportati)
    file_name?: string; url?: string; path?: string; b2_file_name?: string;
    // nuovi campi user-centric
    user_file_id?: number; download_url?: string; presigned_url?: string; id?: number;
  }>> {
  ...
  // (lasciamo il delete com'è: verrà rivisto in seguito)
  deleteUploadPremium(
    userId: number,
    questionarioId: number,
    tipologiaId: number,
  ...
  // =======================
  // Nuovi endpoint (minime aggiunte)
  // =======================

  /** Ritorna i file già caricati dall'utente, raggruppabili per tipologia */
  getUserFilesByTipologia(userId: number) {
    return this.http.get<ApiResponse<Array<{ tipologia_id: number | null; user_file_id: number; filename: string; url?: string }>>>(
      `${this.apiBaseUrl}/user_files_by_tipologia.php?user_id=${userId}&${this.ts()}`
    );
  }

  /** Collega un file esistente (dell'utente) al questionario corrente, senza re-upload */
  attachUserFileToQuestionario(
    ...
    const body = { user_id: userId, questionario_id: questionarioId, user_file_id: userFileId, tipologia_id: tipologiaId ?? null };
    return this.http.post<ApiResponse>(`${this.apiBaseUrl}/attach_file_to_questionario.php?${this.ts()}`, body);
  }
